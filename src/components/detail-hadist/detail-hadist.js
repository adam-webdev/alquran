...
function DetailHadist() {
  ...
  async function getNomorHadist() {
    try {
      const result = await fetch(
        `${baseUrlHadith}/books/${books}/${nomorHadist}`
      );
  ...
  useEffect(() => {
    async function getHadist() {
      try {
        const result = await fetch(
          `${baseUrlHadith}/books/${books}?range=${start}-${end}`
        );
        const json = await result.json();
        if (json.code !== 200) {
          setError(true);
  ...
  useEffect(() => {
    ...
    if (number) {
      ...
      console.log("Fragment URL:", number);
    ...
    <>
      <WrappHeader>
        ...
        <Flex>
          ...
          >
            {hadist &&
              hadist.hadiths.map((hadith, index) => {
                return (
                  <option key={index} value={hadith.number}>
                    Nomor {hadith.number}
                  </option>
                );
              })}
          ...
          <WrappInput>
            <Form onSubmit={handleSubmit}>
              <Input
                value={nomorHadist || ""}
                onChange={(e) => handlechangeNomorHadist(e)}
                type="number"
                placeholder="cari nomor hadist"
                min="1"
                max={hadist && hadist.available}
              />
      ...
      <WrappMain>
            ...
            <Card m_top="30px">
              ...
              {share ? (
                ...
                <SharedIcon left="390px" top="35px">
                  ...
                  <WhatsappShareButton
                    url="http://localhost:3000/detail-surah/2"
                    onClick={handleShare}
                  >
              ...
              <NameHadist>
                <i>
                  {" "}
                  {dataNomorHadist.name} Nomor {dataNomorHadist.contents.number}
                </i>
              </NameHadist>
            </Card>
          )
        ) : loading ? (
          <Skeleton width="100%" height="300px" amount={2} />
        ) : (
          hadist &&
          hadist.hadiths.map((hadith, index) => (
            <Card m_top="30px" key={index} id={hadith.number}>
              ...
              <NameHadist>
                <i>
                  {" "}
                  {hadist.name} Nomor {hadith.number}
                </i>
              </NameHadist>
            </Card>
          ))
        ...
        {nomorHadist ? (
          ...
          <Footer>
            ...
            <Title weight="normal">
              {end} - {hadist && hadist.available} hadist
            </Title>
            <Flex>
              {hadist && end === hadist.available ? (
                <div></div>
              ) : (
                <>
