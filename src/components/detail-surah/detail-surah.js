...
function DetailSurah() {
  ...
  useEffect(() => {
    async function fetchSurah() {
      try {
        const res = await fetch(`${baseUrlQuran}/surah/${surahId}`);
        const json = await res.json();
        console.log("josn detail ", json);
        setData(json.data);
      } catch (error) {
        setError(true);
      } finally {
  ...
  useEffect(() => {
    ...
    if (ayah) {
      ...
      console.log("Fragment URL:", ayah);
      // Contoh: Scroll ke elemen dengan ID yang sesuai dengan fragment
      const element = document.getElementById(ayah);
      if (element) {
        ...
        <Select width="80px" value={ayah} onChange={(e) => handleChangeAyah(e)}>
          {data &&
            data.verses.map((ayah, index) => {
              return (
                <option key={index} value={ayah.number.inSurah}>
      ...
      <WrappMain>
        {loading ? (
          ...
          <>
            ...
            {data.verses.map((ayah, index) => {
              return (
                <Card key={index} id={ayah.number.inSurah}>
                  <CardHeader>
                    {/* <IconMenu /> */}
                  ...
                  <Div>
                    <A>
                      <TextTafsir>Tafsir</TextTafsir>{" "}
                      <Tafsir onClick={handleTafsir} />
                    </A>
                    <Terjemah onClick={handleTerjemah}>Terjemah </Terjemah>
                    <Ayat>
                      {" "}
                      Q.S : {data.name.transliteration.id} ayat{" "}
                      {ayah.number.inSurah}
                    </Ayat>
                  </Div>
